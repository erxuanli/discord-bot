...
class ManagementCmds(commands.Cog):
    ...
    async def addrole(self, ctx, member: discord.Member, role: discord.Role):
        if role.position >= ctx.author.top_role.position:
            await ctx.send(f"You can't add a role higher or equal to your highest role")
        else:
    ...
    async def remrole(self, ctx, member: discord.Member, role: discord.Role):
        if role.position >= ctx.author.top_role.position:
            await ctx.send(f"You can't remove a role higher or equal to your highest role")
        else:
            await ctx.send(f"Permission granted")
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str = None):
        if member.id == ctx.author.id:
            ...
            await ctx.send(f"You can't kick someone with a role, higher or equal to your highest role")
        else:
            await ctx.send(f"Permission granted")
