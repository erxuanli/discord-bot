...
class OtherCmds(commands.Cog):
    ...
    async def nicktimer(self, ctx, t : int = 5): # time in minutes
        if str(ctx.author.id) in self.nicktimer_data[str(ctx.guild.id)]:
            await ctx.send("Already started a timer")
        else:
            if not self.nicktimer_refreshing:
                if t <= 9_999_999_999_999:
                    end_time = time.time() + (t * 60)
                    if str(ctx.guild.id) not in self.nicktimer_data:
                        self.nicktimer_data[str(ctx.guild.id)] = dict()
                    self.nicktimer_data[str(ctx.guild.id)][str(ctx.author.id)] = {"end_time": end_time, "ctx": ctx}
    ...
    async def nicktimer_add(self, ctx, t : int = 1):
        if str(ctx.author.id) not in self.nicktimer_data[str(ctx.guild.id)]:
            await ctx.send("You didn't set a timer")
        else:
            if not self.nicktimer_refreshing:
                if (int((self.nicktimer_data[str(ctx.guild.id)][str(ctx.author.id)]["end_time"] - time.time()) / 60) + t) <= 9_999_999_999_999:
                    self.nicktimer_data[str(ctx.guild.id)][str(ctx.author.id)]["end_time"] = self.nicktimer_data[str(ctx.guild.id)][str(ctx.author.id)]["end_time"] + (t * 60)
    ...
    async def nicktimer_stop(self, ctx):
        if str(ctx.author.id) not in self.nicktimer_data[str(ctx.guild.id)]:
            ...
            if not self.nicktimer_refreshing:
                try:
                    del self.nicktimer_data[str(ctx.guild.id)][str(ctx.author.id)]
    ...
    async def refresh_nicktimers(self):
        ...
        if self.nicktimer_data:
            for server in self.nicktimer_data:
                for user in server:
                    ctx = self.nicktimer_data[server][user]["ctx"]
                    end_time = self.nicktimer_data[server][user]["end_time"]
                    if time.time() >= end_time:
                        ...
                        try:
                            ...
                            pass
        for user in delete:
            del self.nicktimer_data[server][user]
        delete = []
        self.nicktimer_refreshing = False
